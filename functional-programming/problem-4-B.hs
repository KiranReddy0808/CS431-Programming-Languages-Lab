...
check tosolvePuzzle = do                    
    let solution = solve ( array ((0, 0), (8, 8)) ( puzzletuple tosolvePuzzle ))
    printAns solution
...
emptyLocations b = [(r, c) | r <- [0..8], c <- [0..8], b ! (r, c) == 0]   -- Check for empty locations which are denoted by 0
...
b `marksInRow` r = [b ! loc | loc <- range((r, 0), (r, 8))]               -- Checks that the value is not present in the row

marksInColumn ::  Board -> Int -> [Int]                                
b `marksInColumn` c = [b ! loc | loc <- range((0, c), (8, c))]            -- Checks that the value is not present in the column

marksbox :: Board -> (Int,Int) -> [Int]
b `marksbox` (r, c) = [b ! loc | loc <- locations]                        -- Checks that value is not is not present in the box 
  where
    r' = (r `div` 3) * 3                                                  -- we do int/2 *2 which gives even index
    c' = (c `div` 3) * 3
    locations = range((r', c'), (r' + 2, c' + 2))

puzzletuple :: [[Int]] -> [((Int,Int), Int)]                             -- We associate the value at the specific place in the matrix to a 
puzzletuple = concatMap rowtuple . zip [0..8]                           -- tuple of row, column and the value
  where
    rowtuple :: (Int, [Int]) -> [((Int, Int), Int)]
    rowtuple (r, marks) = coltuple r $ zip [0..8] marks                -- Column and the possible mark value
...
printAns (Just b) = mapM_ putStrLn [show ( b `marksInRow` r )| r <- [0..8]]   -- Give the values in the row